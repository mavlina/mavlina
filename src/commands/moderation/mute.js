...
module.exports = {
    ...
    async execute(interactionOrMessage) {
        ...
        if (!member.permissions.has(PermissionsBitField.Flags.MuteMembers)) {
            return interactionOrMessage.reply({ content: 'You do not have permission to mute members.', ephemeral: true });
        }
        ...
        if (!mutedRoleId) {
            return interactionOrMessage.reply({ content: 'The muted role is not configured for this server. Use `/settings mutedrole` to set it.', ephemeral: true });
        }
